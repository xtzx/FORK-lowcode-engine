...
const documentEvents = {
    ...
    'selectionchange': {
        trigger: 'Selection.select/add/remove/clear方法调用',
        data: 'selectedIds数组',
        purpose: 'Selection模块内部状态变更通知'
    }
};
```
...
const detectingEvents = {
    'detectingChange': {
        trigger: 'Detecting.capture/release方法调用',
        data: 'Node | null',
        purpose: '鼠标悬停节点变化通知，用于BorderDetecting组件显示悬停边框',
        constant: 'DETECTING_CHANGE_EVENT'
    }
};
```
...
// 监听 detectingChange 事件
const detectingListener = designer.detecting.onDetectingChange((node) => {
    if (node) {
        ...
        console.log('鼠标离开所有组件');
    }
});

// 监听选择状态的底层变化
const selectionChangeListener = currentDocument.selection.emitter.on('selectionchange', (selectedIds) => {
    console.log('Selection底层状态变化:', {
...
(window as any).debugLowcodeEvents = {
    ...
    listenAll() {
        ...
        designer.detecting.onDetectingChange((node) => {
            console.log(`🎯 Detecting事件: detectingChange`, {
                node: node ? {
                    id: node.id,
                    componentName: node.componentName,
                    title: node.title
